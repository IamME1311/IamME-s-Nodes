...
class PreviewBridge_V2:
    ...
    def INPUT_TYPES(s):
        return {"required":{
                ...
                "optional": {
                    "block": ("BOOLEAN", {"default":False, "label_on": "if_empty_mask", "label_off":"never"})
                    },
    ...
    def doit(self, images, image, unique_id, block=False, prompt=None, extra_pnginfo=None):
        ...
        if not need_refresh:
            pixels, mask, path_item = PreviewBridge_V2.load_image(image)
            image = [path_item]
        else:
